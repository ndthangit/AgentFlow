...
Chạy toàn bộ hạ tầng local bằng Docker: sao chép `.env.example` thành `.env`, thay các mật khẩu, rồi dùng `docker compose up -d --build`. Mở giao diện tại `http://localhost:3000`. Xem [hướng dẫn Docker, Keycloak và PostgreSQL](infra/README.md).
